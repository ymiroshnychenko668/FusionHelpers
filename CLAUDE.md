...
- **Cross-occurrence Cut WITHOUT BaseFeature (verified, the pipe-joint
  ...
  (NOT root) — only the sketch stays in root, so use the **root sketch**
  (`PipeJointSketch_<A>`) as the canonical idempotency marker, never scan
  `rootComponent.features` for the cut. Pin every extrude's
  ...
  `TemporaryBRepManager.createBox(OBB)` plug remains the detection/preview
...
- **Verify against the REAL model, not a generator.** The old synthetic
  ...
  inner-rect end face → ~10 wasted debug cycles. Validate read-only with
  `preview_only=True` and assert `timeline.count`/body volumes unchanged;
  destructive runs only on a `saveAs` scratch copy.
...
- **Pipe Joint Calibration** (`commands/pipe_joint_calibration`,
  ...
  **After calibration**: each pipe body must be single in its own
  ...
  a feature back / unapply the A relief). Detection & `preview_only` stay
  ...
  independent of penetration (verified offset30/dist10, offset5/dist10);
  timeline group all members & collapsed; preview neutral; re-run raises.
